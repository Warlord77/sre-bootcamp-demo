#!/bin/bash
curl -s http://localhost:55009/tasks | jq

# 2. Add a task "Buy milk"
curl -s -X POST http://localhost:55009/tasks \
...
curl -s -X PUT http://localhost:55009/tasks/1 \
     -H "Content-Type: application/json" \
     -d '{"done": true}' | jq

# 6. Update task ID=2 → rename it
curl -s -X PUT http://localhost:55009/tasks/2 \
     -H "Content-Type: application/json" \
...
# 8. Delete task ID=1
#curl -s -X DELETE http://localhost:55009/tasks/1 | jq
